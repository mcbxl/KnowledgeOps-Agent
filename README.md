...
- **证据化运营报告**：自动输出重复候选、低质量文档、潜在冲突、置信度、证据片段和建议动作。
- **Topic Coverage**：统计主题覆盖程度，识别内容薄弱主题、健康主题和高密度主题。
- **检索评估台**：支持输入 benchmark queries，评估 TopK 命中、Top1 分数和引用元数据完整性。
- **前端工作台**：提供导入、检索、问答、运营报告、Agent 工作流、检索评估和知识图谱视图。
...
当前版本刻意保留了可替换的工程边界：Embedding、Rerank、Agent 编排和存储层都可以从本地实现替换为生产级服务。这样既能本地无 API Key 运行，也能在简历和面试中讲清楚升级路径。

- 本地 embedding 用确定性 hashing 实现，便于测试和离线演示。
- 检索链路返回 BM25、Vector、Rerank 三类分数，方便解释召回依据。
- 运营报告中的每个 issue 都带 `confidence`、`evidence` 和 `suggested_actions`，不是简单输出一句“有问题”。
- 知识图谱同时展示 document、section、topic 三类节点，能体现知识组织能力。
